...
export default class Header extends Component {
	...
	handleDeleteProfileModal = () => {
		...
		Modal.confirm({
			...
			content: (
				<React.Fragment>
					Are you sure you want to delete <strong>{profile}</strong>{' '}
					profile?
				</React.Fragment>
			),
			okText: 'Delete',
	...
	handleSaveProfile = () => {
		...
		if (profileList.includes(value)) {
			this.setState({
				modalError:
					'A search profile with the same name already exists',
			});
		} else {
			this.setState(
				{
			...
			>
				...
				>
					...
					>
						...
						/>
						{modalError ? (
							<p style={{ color: 'tomato' }}>{modalError}</p>
						) : null}
					</Modal>
				</div>
			</div>
