...
class RSComponentRender extends Component {
	...
	renderFormItem = (item, name) => {
		...
		if (
			...
			(name === 'showDescription' && !isShopify)
		) {
			return null;
		}
				...
				if (name === 'categoryField') {
					noOptionsMessage = (
						<p style={{ lineHeight: '1.5' }}>
							...
							mappings.{' '}
							<a href={mappingsURL}>You can edit your mappings</a>{' '}
			...
			case 'editor': {
				...
				FormInput = (
					<HtmlEditor
						...
						width="100%"
						value={
							stateComponentProps.customSuggestions ||
							value ||
							undefined
						}
			...
			<Form onFinish={this.handleSubmit} className={formWrapper}>
				<DataFieldInput
					...
					getAvailableDataField={() =>
						getAvailableDataField({ component, id, mappings })
					}
