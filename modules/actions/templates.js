...
export function getAppTemplates() {
	return (dispatch) => {
		...
		return doGet(`${ACC_API}/_templates`)
			.then((res) =>
				dispatch(createAction(AppConstants.APP.TEMPLATES.GET_ALL_SUCCESS, res, null)),
			)
			.catch((error) =>
				dispatch(createAction(AppConstants.APP.TEMPLATES.GET_ALL_ERROR, null, error)),
			);
	};
}
...
export function getAppTemplate(templateName) {
	return (dispatch) => {
		...
		return doGet(`${ACC_API}/_scripts/${templateName}`)
			...
			.catch((error) =>
				dispatch(createAction(AppConstants.APP.TEMPLATES.GET_ERROR, null, error)),
			);
	};
}
...
export function saveAppTemplate(templateName, payload) {
	return (dispatch) => {
		...
		return doPost(`${ACC_API}/_scripts/${templateName}`, payload)
			...
			.catch((error) =>
				dispatch(createAction(AppConstants.APP.TEMPLATES.UPDATE_ERROR, null, error)),
			);
	};
}
...
export function deleteAppTemplate(templateName) {
	return (dispatch) => {
		...
		return doDelete(`${ACC_API}/_scripts/${templateName}`)
			...
			.catch((error) =>
				dispatch(createAction(AppConstants.APP.TEMPLATES.DELETE_ERROR, null, error)),
			);
	};
}
...
export function validateAppTemplate(payload, templateName) {
	return (dispatch) => {
		...
		return doPost(url, payload, undefined, undefined, [404, 400])
			...
			.catch((error) =>
				dispatch(createAction(AppConstants.APP.TEMPLATES.VALIDATE_ERROR, null, error)),
			);
	};
}
